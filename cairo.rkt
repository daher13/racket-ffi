...
(define cairo-create (get-ffi-obj "cairo_create" cairo-lib (_fun _pointer -> _pointer)))
...
(define-cairo cairo-set-font-size (_fun _cairo_t _double -> _void) #:c-id cairo_set_font_size)

(define-cairo cairo-set-source-rgb
              (_fun _cairo_t _double _double _double -> _void)
              #:c-id cairo_set_source_rgb)

(define-cairo cairo-move-to (_fun _cairo_t _double _double -> _void) #:c-id cairo_move_to)

(define-cairo cairo-show-text (_fun _cairo_t _string -> _void) #:c-id cairo_show_text)
